...
OUTPUT_DASHBOARD_FILE = 'hampshire_crime_dashboard.html'
...
def create_dashboard():
    """Creates and displays a crime dashboard for Hampshire."""
    print("Starting dashboard generation for Hampshire...")
    ...
    HeatMap(heat_data, min_opacity=0.3, name="Crime Heatmap").add_to(hampshire_map)
    print("Added heatmap layer to the map.")

    if not street_df.empty:
        street_crime_layer = folium.plugins.MarkerCluster(name="Street Crimes").add_to(hampshire_map)
    ...
    if not stop_search_df.empty:
        stop_search_layer = folium.plugins.MarkerCluster(name="Stop and Search").add_to(hampshire_map)
    ...
    map_html = hampshire_map._repr_html_()
    ...
    <html>
    <head>
        <title>Hampshire Crime Dashboard</title>
    ...
    <body>
        <div class="container">
            ...
            <div class="sidebar">
                <h2>Hampshire Crime Analytics ({DATE})</h2>
